...
public class HomeInventory extends JFrame {
    ...
    JButton newButton = new JButton(new ImageIcon("src/New.gif"));
    JButton deleteButton = new JButton(new ImageIcon("src/Delete.gif"));
    JButton saveButton = new JButton(new ImageIcon("src/Save.gif"));
    JButton previousButton = new JButton(new ImageIcon("src/Previous.gif"));
    JButton nextButton = new JButton(new ImageIcon("src/Next.gif"));
    JButton printButton = new JButton(new ImageIcon("src/Print.gif"));
    ...
    JFileChooser dateDateChooser = new JColorChooser();
    ...
    public  HomeInventory(){
        ...
        int n;
        try{
            BufferedReader inputFile = new BufferedReader(new FileReader("src/inventory.txt"));
    ...
    private Date stringToDate(String s){
        ...
        int d = Integer.valueOf(s.substring(3,5)).intValue()-1;
    ...
    private void locationComboBoxActionPerformed(ActionEvent e){
        ...
        //If not found in list - add to list box
    ...
    private void checkSave(){
        ...
        if(!myInventory[currentEntry -1].description.equals(itemTextField.getText()))
            ...
            edited = true;
        if(edited){
            if(JOptionPane.showConfirmDialog(null,"You have edited this item. Do you want to save the changes?!","Save Changes",JOptionPane.YES_NO_OPTION,JOptionPane.QUESTION_MESSAGE) == JOptionPane.YES_OPTION)
                saveButton.doClick();
        }
    }
    ...
    private void exitForm(WindowEvent evt){
        if(JOptionPane.showConfirmDialog(null, "Any unsaved changes will be lost.\n Are you sure you want to exit?!","Exit Program",JOptionPane.YES_NO_OPTION,JOptionPane.QUESTION_MESSAGE)==JOptionPane.NO_OPTION)
            return;
        try{
            PrintWriter outputFile = new PrintWriter(new BufferedWriter(new FileWriter("src/inventory.txt")));
...
class InventoryDocument implements Printable {
    public int print(Graphics g, PageFormat pf, int pageIndex){
        ...
        int i, iEnd;
        //Here we decide what goes on each page and draw it
        //Header
        ...
        for(i=0+HomeInventory.entriesPerPage * pageIndex; i < iEnd; i++){
            ...
            if(HomeInventory.myInventory[i].marked)
                ...
                g2D.drawString("Item is NOT marked with any indentifying information.", (int)(pf.getImageableX() + 25),y);
            y+=dy;
            ...
            y+=dy;
            try{
                //Maintain original widht/height ratio
                ...
                //Have to go in case image file doesn't open
            }
            y+=2 * dy + 100;
        }
        return PAGE_EXISTS;
